...
Mesh::Mesh(vector<Vector3d> verList, int rows, int cols)
{
    if(verList.size() != rows*cols)
    {
        std::cout<< " rows*cols NOT equal to size of vector" <<std::endl;
    }
    ...
    if(rows < 2 || cols<2)
        std::cout<< " rows/cols less than 2" <<  std::endl;

    vector<Vector3i> triList;
    //triList.resize((rows-1)*(cols-1)*2);
    for(int i=1;i<rows;i++)
    {
        for(int j=0;j<cols-1;j++)
        {
            int id = i*cols+j;
            triList.emplace_back(id, id+1, id+1-cols);
            triList.emplace_back(id, id+1-cols, id-cols);
        }
    }
    triM.resize(triList.size(), 3);
    for(int i=0; i<triList.size(); i++)
...
void Mesh::saveOBJ(string _filename)
{
    fstream ofile;
    ofile.open("../data/"+_filename,std::ios::out);
    if(!ofile)
    {
    ...
    {
        ofile<<"v "<<verM(i,0)<<" "<<verM(i,1)<<" "<<verM(i,2)<<endl;
    }
    int frows = triM.rows();
    for(int i=0;i<frows;i++)
    {
        ofile<<"f "<<triM(i,0)+1<<" "<<triM(i,1)+1<<" "<<triM(i,2)+1<<endl;
    }
